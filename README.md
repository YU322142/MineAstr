...
MineAstr `0.6.29` 是一个 Minecraft 1.21.1 / NeoForge 21.1.219 双端 Mod，用于把 Minecraft 聊天与事件桥接到 AstrBot，并按每位玩家的客户端语言显示 AI 译文。启用原生聊天翻译时会使用未签名消息重发，若需要完整 Secure Chat/原版反刷屏语义，请关闭该选项。

0.6.29 将实体/告示牌/沉浸画框翻译统一到准星目标 HUD：只在当前目标有效时显示译文，移开准星立即清理；默认不重复显示原文，并优先复用 Create 风格的悬浮文本渲染接口。
