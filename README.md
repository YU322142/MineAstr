...
同一个 `mineastr-fabric-0.6.18.jar` 可以放在独立服务端、客户端或两端。服务端只安装 Mod 即可使用聊天、事件、绑定、登录检查及查询；截图功能要求目标玩家客户端也安装该 JAR。
...
产物位于 `build/libs/mineastr-fabric-0.6.18.jar`。

## 安装

1. 为 Minecraft 1.21.11 安装 Fabric Loader。
2. 把 `fabric-api-0.141.4+1.21.11.jar` 放入 `mods`。
3. 把 `mineastr-fabric-0.6.18.jar` 放入 `mods`。
