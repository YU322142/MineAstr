...
MineAstr `v0.6.25` 提供 `image_translate_request` WebSocket 请求。Fabric 客户端
...
AstrBot 配置中的图片提示词最多使用 40000 字；Fabric 外部 Mod 在单次请求中临时传入的
`prompt` 仍受客户端网络载荷的 4096 字符上限约束。
