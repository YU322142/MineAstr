...
- 普通聊天、Minecraft 原生聊天和告示牌翻译只召回当前原文命中的术语；完整词典不会整体进入提示词。
- 沉浸画框图片翻译先完成 OCR 与初译；OCR 原文命中词典时再使用精确术语进行第二段校正，未命中时保持单次模型调用。
- 图片 OCR 与术语校正共用 27 秒协议预算；校正超时、失败或返回空译文时保留首轮有效译文，不再撞上 Mod 的 30 秒等待上限。
- 图片与告示牌模型请求改为受控后台任务，翻译期间 WebSocket 仍可处理心跳和查询；同一连接同时只运行一个翻译请求，避免模型请求洪泛。
- 图片缓存现在纳入词典开关、显示原文、长度限制和模型选择；相关配置热更新后不会继续复用旧结果。
- 外置词表支持 `en_us` 与 `zh_cn` 双向精确约束；同一显示名对应多个冲突译名时不会强制猜测，只有注册键提供上下文时才精确套用。
- 词典按文件大小和修改时间热重载；路径为空、文件缺失、JSON 损坏、文件过大或条目异常时安全禁用词典并继续原有翻译流程。
- 新增配置页字段、部署说明及词典加载/热更新/边界测试；无需修改 MineAstr NeoForge 客户端或服务端协议。
- 提供可复现的语言目录转词典脚本；推荐词典会过滤多数 tooltip、description、condition 等说明文本，并用最长名称优先匹配避免短词污染。
...
- Keep targeted sign overlays and normalized duplicate-text handling from the previous release line.
